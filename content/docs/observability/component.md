...
**{{< brand-icon name="prometheus" alt="Prometheus" >}} Prometheus** 负责采集和存储指标。DevFlow 所有服务都暴露 `/metrics` 端点，Prometheus 定期来"查房"。

**{{< brand-icon name="grafana" alt="Grafana" >}} Grafana** 负责展示。预置了 10+ 个仪表盘：
- 服务概览（QPS、延迟、错误率）
- 发布流水线（构建、渲染、部署各阶段耗时）
- 运行时状态（Pod 状态、资源使用）
...
**📘 Loki** 负责存储和查询日志。不像传统 ELK 那么重，Loki 只索引标签，不索引日志内容，更省资源。
...
**{{< brand-icon name="opentelemetry" alt="OpenTelemetry" >}} OpenTelemetry** 负责采集链路数据。一个请求从进来到出去，经过了哪些服务、每个服务花了多少时间，全部记录下来。
