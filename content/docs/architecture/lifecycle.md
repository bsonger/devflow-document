...
更重要的是，发布步骤本身是由 Tekton pipeline task 组合出来的，所以 operator 可以通过调整 task 来改变发布动作，而不需要改 release-service 的核心代码。
...
- **步骤可编排**: 发布步骤由 pipeline task 组合，步骤变化优先通过配置和 task 复用实现

这也是为什么同一套 release 流程可以在不改代码的前提下，插入审批、扫描、验证或回滚前置检查。
